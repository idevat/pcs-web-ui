...
cluster storage** (not in a task wizard), the preferred pattern is a set of
dedicated actions with a matching saga — rather than reusing
`LIB.CALL.CLUSTER.TASK`. Established example: `RESOURCE_AGENT.LOAD` /
`.LOAD.SUCCESS` / `.LOAD.FAILED` with a saga in `sagas/resourceAgent.ts`.
